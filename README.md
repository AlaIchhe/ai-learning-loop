...
```bash
python -m pytest tests/ -v    # 126 个用例，Mock LLM，无需真实 API
```
...
│   ├── state.py             # AgentState（6 持久字段 + 5 轮次缓存 + NodeOutput）
...
│   └── referee.py           # 裁判：双策略拼合（with_structured_output / JSON-mode）+ 判定 + 总结
├── workflow/                # 编排层
│   └── graph.py             # LangGraph 图组装（8 节点）、条件路由、export_graph()
├── ui/                      # 展现层
│   └── app.py               # Streamlit 界面（动态中断 UI、路径自适应 .env 加载）
├── tests/                   # 测试（Mock LLM，126 个用例）
│   ├── helpers.py           # 共享工厂函数（make_state / make_mock_model）
...
│   ├── test_interfaces.py   # 28 个用例：跨层接口、序列化、checkpoint、路由、prompt 边界
...
- **统一环境初始化**：`core/env.py` 的 `setup_environment()` 消除 `run.py` 和 `scripts/*.py` 中的三重初始化代码重复
...
- **LLM 调用自动重试**：`invoke_with_retry()` 对所有瞬时错误（网络/超时/速率限制）自动重试 3 次，指数退避（1s/2s/4s）
