...
│   ├── rxweb.py            # 应用入口（路由注册）
│   └── styles.py           # 全局样式系统：多标签页 + 流式输出 + 温度调节；复用 setup_environment + has_configured_api_key
：inject_global_css() 加载 CSS + 自动滚动 JS + typing_cursor_html()
│   └── assets/fonts.css    # 字体与 Markdown 样式（Inter 字体、消息气泡、按钮动画、暗色模式、滚动条美化）
├── rxconfig.py              # Reflex 配置（端口、插件、transport）（明/暗双主题、学术蓝品牌色、隐藏默认菜单）
├── tests/                   # 测试（Mock LLM，需真实 API，154 个用例）
...
│   └── ci.yml               # 自动运行 pytest + ruff + pyright + mypy（真实 API Key）
