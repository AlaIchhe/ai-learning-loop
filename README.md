...
基于 LangGraph 的三智能体苏格拉底式学习系统。**提问者**通过苏格拉底式追问探询学习主题的边界与隐含前提、**精确化者**将用户的回应转化为精确表述、**引导者**将每轮对话揭示的新认知层次有机拼合到理解中（一句话逐步生长为一段话）。Streamlit 提供动态中断人机协作界面，LangSmith 提供全链路可观测性。
...
使用真实 API Key 测试完整系统（无 Mock）。覆盖 Questioner / Refiner / Guide 单 Agent 有效性、LangGraph 完整单轮/多轮工作流、Checkpoint 持久性。**适配 DeepSeek**：引导者直接使用生产代码 `referee_deliberate_node(json_mode=True)`，无需重复实现。
...
> **DeepSeek 用户注意**：DeepSeek 不支持 `with_structured_output`（底层依赖 `response_format`）。系统已内置 JSON-mode 兼容策略，引导者节点通过 `json_mode=True` 参数自动切换到提示词引导 + 正则提取 + Pydantic 验证的方式，无需手动适配。
...
│   ├── state.py             # AgentState（9 持久字段 + 5 轮次缓存）+ make_initial_state + validate_state_shape + NodeOutput
│   ├── schemas.py           # Pydantic 结构化模型（_StrictModel 基类，extra='forbid'）
│   ├── prompts.py           # System Prompt 与模板函数（提问/精确化/拼合/总结）
│   ├── model.py             # ModelConfig + load_model_config + has_configured_api_key + get_chat_model（支持 per-tab 覆盖）
│   └── logging.py           # 结构化日志：trace_id_context + TraceLogger（LLM 计时/重试计数/错误记录）+ JSON 行输出
├── agents/                  # 智能体节点（无状态纯函数，compute + interact 拆分）
│   ├── _base.py             # 共享 LLM 工具（内容提取/消息构造/调用重试）
│   ├── opponent.py          # 提问者：苏格拉底式追问 → question（compute）/ interrupt 展示（interact）
│   ├── presenter.py         # 精确化者：用户回应 → draft（compute）/ interrupt 确认（interact）
│   └── referee.py           # 引导者：双策略认知拼合 + _build_round_record / _dump_round_record 归档辅助 + 判定 + 总结
├── workflow/                # 编排层
...
│   └── integration_test_real.py  # 真实 API 集成测试（6 个用例，复用生产引导者节点）
...
- **最大轮次安全阀**：`_route_after_referee` 在 `round >= max_rounds` 时强制终止，防止异常状态下无限循环。`max_rounds` 由侧边栏滑块设置（1–20，默认 10），启动时捕获到 `AgentState.max_rounds`
...
- **流式输出机制**：按钮回调仅设置 `pending_start`/`pending_resume` 标记，实际 `graph.stream()` 在渲染线程中执行，`st.empty()` 占位符渐进更新。`GraphInterrupt` 捕获中断点
