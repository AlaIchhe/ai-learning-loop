...
def message_bubble(message: dict) -> rx.Component:
    ...
    avatar = rx.match(
        role, ("user", "👤"), ("questioner", "⚔️"),
        ("refiner", "✨"), ("guide", "🧠"), "📋",
    )
    name = rx.match(
        role, ("user", "你"), ("questioner", "提问者"),
        ("refiner", "提炼者"), ("guide", "引导者"), "系统",
    )
    ...
    return rx.box(
        rx.hstack(
            ...
            rx.box(
                ...
                class_name="message-content", align_items=rx.cond(is_user, "flex-end", "flex-start"),
            ),
            class_name=rx.cond(is_user, "chat-message user-message", "chat-message ai-message"), spacing="3",
        ), width="100%",
    )
    ...
    return rx.center(
        rx.vstack(
            ...
            rx.text(
                ...
                size="4", color=colors["text_secondary"], align="center",
            ...
            rx.box(
                rx.input(
                    ...
                    size="3", width="100%", radius="large",
                ),
                rx.button(
                    "开始探讨",
                    on_click=AppState.start_debate,
                    size="3", width="100%", margin_top="1rem", color_scheme="blue",
                    disabled=AppState.user_input.strip() == "",
                ),
                width="100%", style={"max_width": "28rem"}, margin_top="2rem",
            ),
            rx.box(
                rx.vstack(
                    ...
                    rx.slider(
                        ...
                        min=0.0, max=1.5, step=0.1, width="100%",
                    ),
                    rx.text(AppState.agent_temperature.to_string(), size="1", color=colors["text_secondary"]),
                    spacing="1",
                ),
                width="100%", style={"max_width": "28rem"}, margin_top="1.5rem",
            ),
...
def active_chat_view() -> rx.Component:
    ...
    return rx.vstack(
        rx.hstack(
            ...
            width="100%", padding_y="1rem", border_bottom=f"1px solid {colors['border']}",
        ),
        rx.box(
            rx.vstack(
            ...
            class_name="chat-container", width="100%", flex_grow=1, padding_y="1rem",
        ),
        rx.box(
            rx.box(
                rx.hstack(
                    rx.text_area(placeholder="输入你的回应...", value=AppState.user_input,
                                 on_change=AppState.set_user_input, size="3", radius="large",
                                 min_height="80px", width="100%"),
                    rx.button("发送", on_click=AppState.submit_user_response, size="3",
                              color_scheme="blue", disabled=AppState.user_input.strip() == ""),
                    align_items="flex-end", width="100%", class_name="input-container",
                ),
                display=rx.cond(AppState.active_awaiting_user_response, "block", "none"),
            ),
            rx.box(
                ...
                class_name="input-container", padding_y="1rem",
            ),
...
def sidebar() -> rx.Component:
    return rx.box(
        rx.vstack(
            ...
            rx.hstack(
                rx.button(
                    "+", on_click=AppState.add_tab, variant="ghost", size="2",
                    title="新建会话",
                ),
                rx.button(
                    "✕", on_click=AppState.remove_active_tab, variant="ghost", size="2",
                    title="关闭当前会话",
                    disabled=AppState.tabs.length() <= 1,
                ),
                width="100%", justify="center", spacing="1",
            ),
            rx.spacer(),
            # 模型设置链接
            rx.el.a(
                rx.hstack(rx.icon("settings"), rx.text("模型设置"), width="100%",
                          padding="0.75rem", border_radius="0.5rem",
                          _hover={"background_color": colors["border"]}),
                href="/settings", width="100%", text_decoration="none", color="inherit",
            ),
            # 主题切换
            rx.hstack(
                ...
                justify="between", width="100%", padding="0.75rem",
            ),
            align_items="flex-start", height="100%", width="100%",
        ),
        class_name="sidebar",
    )
...
def chat_page() -> rx.Component:
    ...
    return rx.hstack(
        ...
        rx.box(
            ...
            class_name="main-content", margin_left="280px", width="100%", height="100vh",
        ),
        width="100%", height="100vh", on_mount=AppState.initialize,
    )
